...
# Zapytanie GraphQL
QUERY='{
  "query": "query { group(fullPath: \"pl.rachuna-net/cicd/components\") { projects(includeSubgroups: true) { nodes { name fullPath description } } } }"
}'
...
{
  echo "# Lista komponentów CI/CD"
  echo ""
  echo "| component | version | description |"
  echo "|-----------|---------|-------------|"

  # Parsowanie, sortowanie i generowanie tabelki
  echo "$RESPONSE" | jq -r '.data.group.projects.nodes | sort_by(.name)[] | "| [\(.name)](https://gitlab.com/\(.fullPath)) | ![](https://gitlab.com/\(.fullPath)/-/badges/release.svg) | \(.description) |"'
} > "$OUTPUT_FILE"
